...
CausalArima<-function(y, auto = TRUE, order = c(0, 0, 0), seasonal = c(0, 0, 0), ic = "aic", xreg = NULL, dates, 
  ...
  if(!identical(length(y),length(dates))){stop("'y' and 'dates' lengths differ")}
  ...
  ### STEP 2. Forecasting the counterfactual outcome in the absence of intervention
  h<-length(y.01)
  ...
  ### STEP 3. Causal effect computation: direct comparison between the observed outcome (y.01) and the
  #           predicted counterfactual (mean.fcast)
  causal.effect.0 <- y.01 - mean.fcast.0 
  
  ### STEP 4. Estimation of variance, confidence intevals and tstats
