...
plot.cArima <- function(x, type = c("forecast", "impact", "residuals"), horizon = NULL, ...) {
  ...
  # param checks
  if (class(x) != "cArima")
    stop("`x` must be an object of class cArima")
  ...
  if (any(horizon <= x$int.date))
    stop("Dates in `horizon` must follow `int.date`")

  # Plot 'Observed vs Forecast'
  if ("forecast" %in% type) {
    res_1 <- .forecast(x, horizon = horizon, ...)
  ...
  # Plot 'Causal effect'
  if ("impact" %in% type) {
    res_2 <- .impact(x, horizon = horizon, ...)
  ...
  return(list(forecast = res_1, impact = res_2, residuals = res_3))
}

# -----------------------------------------------------------------------------------------

.impact <- function(cArima, horizon = NULL, alpha = 0.05, color_line = "darkblue", color_intervals = "slategray2",
                    lines_size = 0.4) {
  ...
  g <- ggplot(data = dat, aes(x = x)) + coord_cartesian(ylim = ylim) +
              ...
              geom_line(aes(y = y), color = color_line, linetype = "solid", size = lines_size)
  ...
  g_cum <- ggplot(data = dat_cum, aes(x = x)) + labs(title = "Cumulative effect", y = "", x = "") +
                  theme_bw(base_size = 15) + geom_ribbon(aes(x = x, ymax = y.upper, ymin = y.lower), fill = color_intervals) +
                  geom_hline(yintercept = 0, colour = "darkgrey", size = lines_size, linetype = "solid") +
                  geom_line(aes(y = y), color = color_line, linetype = "solid", size = lines_size)

  if (!is.null(horizon)) {

    g <- g + sapply(as.Date(horizon), function(xint) geom_vline(aes(xintercept = xint, linetype = "Time horizon"))) +
             scale_linetype_manual(values = "dotdash") + labs(linetype = " ")
    g_cum <- g_cum + sapply(as.Date(horizon), function(xint) geom_vline(aes(xintercept = xint, linetype = "Time horizon"))) +
                     scale_linetype_manual(values = "dotdash") + labs(linetype = " ")

  }

  results <- list(point_effect = g, cumulative_effect = g_cum)
  return(results)
...
.forecast <- function(cArima, horizon = NULL, win = 0.4, colours = c("darkblue", "black"), fill_colour = "slategray2",
                      lines_size = 0.6) {
  ...
  g <- ggplot(data = dat, aes(x = x, colour = "Legend")) + coord_cartesian(ylim = ylim) + theme_bw(base_size = 15) +
              labs(title = "Forecasted series", y = "", x = "") + scale_colour_manual(values = colours) +
              geom_vline(aes(xintercept = int.date, linetype = paste(int.date)), colour = "darkgrey", size = lines_size) +
              scale_linetype_manual(values = "longdash") + labs(color = "Time series", linetype = "Intervention date") +
...
.residuals <- function(cArima, max_lag = 30) {
  ...
  # Normal QQ plot
  QQ_plot <- qqplot.data(std.res) + ggtitle("Normal Q-Q Plot") + xlab("Theoretical Quantiles") + ylab("Sample Quantiles") +
             theme_bw(base_size = 15)
